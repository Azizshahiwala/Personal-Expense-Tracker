...
interface Message {
  id: string;
  type: MessageType; // ← determines layout & styling
  sender_id: string;
  sender_name: string;
  message: string;
  timestamp: string;
    ...
    if (response.ok) {
      ...
      const formatted = data.history.map((msg: any, index: any) => {
        ...
        return {
          ...
          type: "user" as MessageType, // ← all history is user messages
          sender_id: senderId,
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      const incomingMessage: Message = {
        ...
        type: "user" as MessageType, // ← websocket is always user messages
